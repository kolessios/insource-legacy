...
CUsersRecipientFilter::CUsersRecipientFilter( int team, bool ignoreBots, bool isReliable ) : CTeamRecipientFilter( team, isReliable )
{
    for ( int i = 0; i < GetRecipientCount(); ++i ) {
        CBasePlayer *pPlayer = ToBasePlayer( UTIL_PlayerByIndex( GetRecipientIndex( i ) ) );
