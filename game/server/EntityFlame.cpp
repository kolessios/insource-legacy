...
void CEntityFlame::FlameThink( void )
{
	...
	if ( m_hEntAttached )
	{
		...
		if( !m_hEntAttached->IsNPC() && hl2_episodic.GetBool() )
		{
			...
			CSoundEnt::InsertSound( SOUND_DANGER | SOUND_CONTEXT_FROM_FIRE, GetAbsOrigin(), ENTITYFLAME_MOVE_AWAY_DIST, 0.1f, this, SOUNDENT_CHANNEL_REPEATED_DANGER );
			CSoundEnt::InsertSound( SOUND_DANGER | SOUND_CONTEXT_FROM_FIRE, GetAbsOrigin() + Vector( 0, 0, 48.0f ), ENTITYFLAME_MOVE_AWAY_DIST, 0.1f, this, SOUNDENT_CHANNEL_REPEATING );
		}
	}
	else
	{
