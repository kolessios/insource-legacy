...
CAI_Hint *CAI_HintManager::FindHint( CAI_BaseNPC *pNPC, const Vector &position, const CHintCriteria &hintCriteria )
{
    ...
    //if ( listCount == 1 )
        //return lists.Element( 0 )->Element( 0 );

    if ( listCount == 1 ) {
        for ( int it = 0; it < lists.Element( 0 )->Count(); it++ ) {
            CAI_Hint *pTestHint = lists.Element( 0 )->Element( it );
...
hinttypedescs_t g_pszHintDescriptions[] =
{
	...
	{	HINT_TACTICAL_COVER, "Tactical: Cover"	},
