...
extern ConVar nav_show_blocked;
    ...
    if ( nav_show_blocked.GetBool() )
    {
        FOR_EACH_VEC( TheNavAreas, it )
        {
            ...
            if ( area->IsBlocked( TEAM_ANY ) ) {
                area->DrawFilled( 100, 100, 200, 155 );
            }
            else if ( area->HasAvoidanceObstacle() ) {
                area->DrawFilled( 100, 200, 100, 155 );
            }
        }
    }
...
bool CNavMesh::GetGroundHeight( const Vector &pos, float *height, Vector *normal ) const
{
	...
	while( to.z - pos.z < flMaxOffset ) 
	{
		...
		if ( check > 5000 )
		{
			Assert( !"Loop in GetGroundHeight!!" );
			break;
		}
...
NavAttributeLookup TheNavAttributeTable[] =
{
	...
	{ "HIDDEN", (NavAttributeType)NAV_MESH_HIDDEN },

	#ifdef APOCALYPSE
	{ "SPAWN_AMBIENT", (NavAttributeType)NAV_MESH_SPAWN_AMBIENT },
	{ "SPAWN_FORCED", (NavAttributeType)NAV_MESH_SPAWN_FORCED },
	{ "SPAWN_BOSS", (NavAttributeType)NAV_MESH_SPAWN_BOSS },
	{ "SPAWN_SOLDIERS", (NavAttributeType)NAV_MESH_SPAWN_SOLDIERS },
	{ "RESOURCES", (NavAttributeType)NAV_MESH_RESOURCES },
	#endif
