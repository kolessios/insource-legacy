...
private:
    ...
    CNetworkVar( bool, m_bHighQualityShadows );
    CNetworkVar( float, m_flShadowsFilter );
};
...
BEGIN_DATADESC( CGlobalLight )
    ...
    DEFINE_KEYFIELD( m_bHighQualityShadows, FIELD_BOOLEAN, "shadowshighquality" ),
    DEFINE_KEYFIELD( m_flShadowsFilter, FIELD_FLOAT, "shadowsfilter" ),
...
IMPLEMENT_SERVERCLASS_ST_NOBASE(CGlobalLight, DT_GlobalLight)
    ...
    SendPropBool( SENDINFO( m_bHighQualityShadows ) ),
    SendPropFloat( SENDINFO( m_flShadowsFilter ) ),
END_SEND_TABLE()
...
CGlobalLight::CGlobalLight()
{
    ...
    m_bHighQualityShadows = 0;
    m_flShadowsFilter = -1.0f;
}
