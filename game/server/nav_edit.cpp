		...
		if (m_selectedArea)
		{
			...
			if (m_showAreaInfoTimer.HasStarted() && !m_showAreaInfoTimer.IsElapsed() )
			{
				...
				else
				{
					...
					if ( attributes & NAV_MESH_DONT_SPAWN )		Q_strncat( attrib, "DONT SPAWN ", sizeof( attrib ), -1 );
					if ( attributes & NAV_MESH_PLAYER_START )		Q_strncat( attrib, "PLAYER START ", sizeof( attrib ), -1 );
					if ( attributes & NAV_MESH_HIDDEN )		Q_strncat( attrib, "HIDDEN ", sizeof( attrib ), -1 );

					#ifdef APOCALYPSE
					if ( attributes & NAV_MESH_SPAWN_AMBIENT )		Q_strncat( attrib, "SPAWN AMBIENT ", sizeof( attrib ), -1 );
					if ( attributes & NAV_MESH_SPAWN_FORCED )		Q_strncat( attrib, "SPAWN FORCED ", sizeof( attrib ), -1 );
					if ( attributes & NAV_MESH_SPAWN_BOSS )		Q_strncat( attrib, "SPAWN BOSS ", sizeof( attrib ), -1 );
					if ( attributes & NAV_MESH_SPAWN_SOLDIERS )		Q_strncat( attrib, "SPAWN SOLDIERS ", sizeof( attrib ), -1 );
					if ( attributes & NAV_MESH_RESOURCES )		Q_strncat( attrib, "RESOURCES ", sizeof( attrib ), -1 );
					#endif
