...
ConVar nav_area_max_size( "nav_area_max_size", "10", FCVAR_CHEAT, "Max area size created in nav generation" );
...
bool CNavMesh::UpdateGeneration( float maxTime )
{
	...
	switch( m_generationState )
	{
		...
		case SAVE_NAV_MESH:
		{
            ...
            engine->ServerCommand( "ai_inhibit_spawners 0\n" );
...
bool IsWalkableTraceLineClear( const Vector &from, const Vector &to, unsigned int flags )
{
	...
	for( int t=0; t<maxTries; ++t )
	{
		...
		UTIL_TraceHull( useFrom, to, Vector( -2, -2, 0 ), Vector( 2, 2, 10.0f ), MASK_PLAYERSOLID, &traceFilter, &result );
...
//--------------------------------------------------------------------------------------------------------------
/**
* Temp way to mark cliff areas after generation without amount'ing.  Any area that is adjacent to a cliff
