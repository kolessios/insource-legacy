...
{
    if ( IsSoundType( SOUND_CONTEXT_FORCE_FROM_OWNER ) ) {
...
{
    if ( IsSoundType( SOUND_CONTEXT_GUNFIRE | SOUND_CONTEXT_REACT_TO_SOURCE | SOUND_CONTEXT_INJURY | SOUND_CONTEXT_FROM_SNIPER ) ) {
        if ( m_hOwner.Get() )
            return m_hOwner.Get()->GetAbsOrigin();
    }

    // TODO: Quizá debamos devolver el dueño del impacto de una bala, sin embargo
    // esto le quitaria realismo al juego...
...
int CSoundEnt::InsertSound ( int iType, const Vector &vecOrigin, int iVolume, float flDuration, CBaseEntity *pOwner, int soundChannelIndex, CBaseEntity *pSoundTarget )
{
	...
	if( displaysoundlist.GetInt() == 1 )
	{
        ...
        NDebugOverlay::Text( pSound->GetSoundOrigin(), UTIL_VarArgs( "Sound: %d", pSound->SoundType() ), false, flDuration );
	}
