...
// Purpose: Simple class to create a filter for a single player ( unreliable )
//-----------------------------------------------------------------------------
class CUserAndObserversRecipientFilter : public CRecipientFilter
{
public:
    CUserAndObserversRecipientFilter( CBasePlayer *player, int observerMode = OBS_MODE_IN_EYE )
    {
        AddRecipient( player );

        for ( int it = 0; it <= gpGlobals->maxClients; ++it ) {
            CBasePlayer *pPlayer = UTIL_PlayerByIndex( it );
            ...
            if ( pPlayer->GetObserverTarget() == player && pPlayer->GetObserverMode() == observerMode )
...
// Purpose: Simple class to create a filter for all players on a given team 
//-----------------------------------------------------------------------------
class CUsersRecipientFilter : public CTeamRecipientFilter
{
public:
    CUsersRecipientFilter( int team = TEAM_ANY, bool ignoreBots = true, bool isReliable = false );
};
...
// Purpose: Simple PVS based filter ( unreliable )
//-----------------------------------------------------------------------------
class CHumanPVSFilter : public CRecipientFilter
{
public:
    CHumanPVSFilter( const Vector& origin )
    {
        AddRecipientsByPVS( origin );

        for ( int s = 0; s < GetRecipientCount(); ++s ) {
            CBasePlayer *pPlayer = UTIL_PlayerByIndex( GetRecipientIndex( s ) );
