...
void CBot::DebugDisplay()
{
	...
	float thinkTime = (Plat_FloatTime() - m_flStartTime) * 1000.0f;
	...
	if (GetActiveSchedule()) {
		...
		DebugScreenText(msg.sprintf("    Schedule: %s", g_BotSchedules[pSchedule->GetID()]));
	...
	if (bot_debug_desires.GetBool()) {
		...
		FOR_EACH_MAP(m_nSchedules, it)
		{
			...
			float computedDesire = pItem->GetComputedDesire();
			Color desireColor = white;

			if (computedDesire >= BOT_DESIRE_VERYHIGH || pItem == GetActiveSchedule()) {
				desireColor = red;
			}
			else if (!pItem->ShouldCalculateDesire()) {
				desireColor = Color(222, 222, 222, 255);
			}
			else if (computedDesire == BOT_DESIRE_HIGH) {
				desireColor = pink;
			}
			else if (computedDesire == BOT_DESIRE_MODERATE) {
				desireColor = yellow;
			}
			else if (computedDesire > BOT_DESIRE_NONE) {
				desireColor = green;
			}

			DebugScreenText(msg.sprintf("    %s = %.2f", g_BotSchedules[pItem->GetID()], computedDesire), desireColor);
		}
	}
