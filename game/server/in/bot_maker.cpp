...
void CBotSpawn::PreparePlayer()
{
    ...
    if ( m_bDisabledMovement )
        pAI->SetNavigationDisabled( m_bDisabledMovement );

    if ( m_iBlockLookAround > 0 && pAI->Aim() )
        pAI->Aim()->BlockLookAround( m_iBlockLookAround );

    if ( m_nFollowEntity != NULL_STRING )
        pAI->StartFollow( STRING( m_nFollowEntity ) );
    ...
    if ( HasSpawnFlags( SF_USE_SPAWNER_POSITION ) ) {
        pAI->m_vecSpawnSpot = GetAbsOrigin();
...
void CBotSpawn::InputBlockLook( inputdata_t &inputdata ) 
{
    ...
    if ( GetBot() && GetBot()->GetAI() && GetBot()->GetAI()->Aim() )
        GetBot()->GetAI()->Aim()->BlockLookAround( m_iBlockLookAround );
}
...
void CBotSpawn::InputDisableMovement( inputdata_t &inputdata ) 
{
    ...
    if ( GetBot() && GetBot()->GetAI() && GetBot()->GetAI()->Navigation() )
        GetBot()->GetAI()->Navigation()->SetDisabled( m_bDisabledMovement );
}
