    ...
    if ( member ) {
        ...
        FOR_EACH_MEMBER(
        {
            ...
            if ( pMember->GetAI() && ShouldFollowLeader() )
                pMember->GetAI()->StartFollow( member );
        });
...
bool CSquad::IsSomeoneLooking( CBaseEntity * pTarget, CPlayer *pIgnore )
{
    FOR_EACH_VEC( m_nMembers, it )
    {
        ...
        if ( pMember->GetAI() && pMember->GetAI()->GetLookTarget() ) {
            if ( pMember->GetAI()->GetLookTarget() == pTarget )
                return true;
...
bool CSquad::IsSomeoneLooking( const Vector & vecTarget, CPlayer *pIgnore )
{
    FOR_EACH_VEC( m_nMembers, it )
    {
        ...
        if ( pMember->GetAI() && pMember->GetAI()->GetLookingAt().IsValid() ) {
            if ( pMember->GetAI()->GetLookingAt() == vecTarget )
                return true;
...
bool CSquad::IsSomeoneGoing( const Vector & vecDestination, CPlayer *pIgnore )
{
    FOR_EACH_VEC( m_nMembers, it )
    {
        ...
        if ( pMember->GetAI()->GetDestination() == vecDestination )
            return true;
    }
