...
//================================================================================
// Establece las ID's de los huesos dependiendo de la entidad
//================================================================================
bool Utils::GetEntityBones( CBaseEntity *pEntity, HitboxBones &bones )
{
    ...
    Assert( !"Invalid Entity in Utils::GetEntityBones" );
    return false;
}

//================================================================================
// Establece vecOrigin con la posición del Hitbox especificado de la entidad.
//================================================================================
bool Utils::GetHitboxPosition( CBaseEntity *pEntity, Vector &vecPosition, HitboxType type )
{
    HitboxPositions positions;

    // Obtenemos la posición de cada hitbox
    if ( !GetHitboxPositions(pEntity, positions) )
        return false;

    // Invalidamos
    vecPosition.Invalidate();

    // Establecemos la posición deseada
    switch( type )
    {
        case HITGROUP_HEAD:
...
//================================================================================
// Establece positions con cada una de las posiciones de los Hitbox de la entidad
// ¡¡Esto es pesado para el motor!! Usar con cuidado.
//================================================================================
bool Utils::GetHitboxPositions( CBaseEntity *pEntity, HitboxPositions &positions )
{
    ...
    // Obtenemos información de los huesos
    HitboxBones bones;

    // Verificamos si al menos tiene huesos
...
//================================================================================
bool Utils::IsValidSpot( const Vector & vecSpot, const Vector & vecOrigin, const CSpotCriteria & criteria, CPlayer * pPlayer )
{
    ...
    if ( criteria.m_bOutOfLineOfFire && IsCrossingLineOfFire( vecOrigin, vecSpot, pPlayer ) ) {
        return false;
    }

    // Tenemos que evitar escondites cerca del equipo enemigo
    if ( criteria.m_iAvoidTeam && IsSpotOccupied( vecSpot, pPlayer, criteria.m_flMinDistanceFromEnemy, criteria.m_iAvoidTeam ) ) {
        return false;
    }
    ...
    // No es visible
    if ( criteria.m_bOnlyVisible && pPlayer && !pPlayer->IsAbleToSee( vecSpot, CBaseCombatCharacter::DISREGARD_FOV ) ) {
        return false;
    }
