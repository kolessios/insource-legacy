...
void CAI_BaseNPC::Event_Killed( const CTakeDamageInfo &info )
{
	...
	// Terminamos cualquier tarea actual
	TaskComplete();
...
{
	if ( pSound->SoundContext() & SOUND_CONTEXT_PLAYER_ALLIES_ONLY )
	{
		if ( !IsPlayerAlly() )
			return false;
	}
...
NPC_STATE CAI_BaseNPC::SelectIdleIdealState()
{
		  ...
		  HasCondition(COND_HEAR_BULLET_IMPACT) )
	{
		...
		if ( pSound )
		{
			...
			if ( pSound->IsSoundType( SOUND_COMBAT | SOUND_DANGER ) )
			{
				return NPC_STATE_ALERT;
			}
		}
...
{
	CSoundEnt::InsertSound( SOUND_COMBAT, EyePosition(), volume, duration, this, SOUNDENT_CHANNEL_FOOTSTEP );
}
