...
#define TRACER_SOUND_TIME_MIN	0.2f
...
ConVar cl_tracer_whiz_distance( "cl_tracer_whiz_distance", "24" );	// putting TRACER_MAX_HEAR_DIST on a cvar, so KellyT can find a good value
...
void FX_TracerSound( const Vector &start, const Vector &end, int iTracerType )
{
    ...
    C_Player *pPlayer = (C_Player *)C_BasePlayer::GetLocalPlayer( GET_ACTIVE_SPLITSCREEN_SLOT() );
