...
void CBaseClientRenderTargets::CreateGlassReflectionTextures( IMaterialSystem* pMaterialSystem, int iSize )
{
    iSize = CommandLine()->ParmValue( "-reflectionTextureSize", iSize );

    // Creamos una textura para cada reflejo
    for ( int it = 0; it < MAX_REFLECTIONS; ++it )
    {
        ...
        Assert( !IsErrorTexture( m_GlassReflectionTexture[it] ) );
    }    
}

void CBaseClientRenderTargets::CreateGlassRefractionTextures( IMaterialSystem * pMaterialSystem, int iSize )
{
    iSize = CommandLine()->ParmValue( "-reflectionTextureSize", iSize );
    ...
    for ( int it = 0; it < MAX_REFLECTIONS; ++it )
    {
        // _rt_GlassRefraction_0
        // _rt_GlassRefraction_1
        m_GlassRefractionTexture[it].Init( pMaterialSystem->CreateNamedRenderTargetTextureEx2(
            UTIL_VarArgs( "_rt_GlassRefraction_%i", it ),
            iSize, iSize, RT_SIZE_PICMIP,
            pMaterialSystem->GetBackBufferFormat(),
            MATERIAL_RT_DEPTH_SHARED,
            TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT,
            CREATERENDERTARGETFLAGS_HDR ) );
        Assert( !IsErrorTexture( m_GlassRefractionTexture[it] ) );
    }
}
...
void CBaseClientRenderTargets::SetupClientRenderTargets( IMaterialSystem* pMaterialSystem, IMaterialSystemHardwareConfig* pHardwareConfig, int iWaterTextureSize, int iCameraTextureSize )
{
    ...
    // Glass Reflection Textures!
    CreateGlassReflectionTextures( pMaterialSystem );
    CreateGlassRefractionTextures( pMaterialSystem );
