...
static ConVar cl_bones_merge("cl_bones_merge", "1", FCVAR_CHEAT);
static ConVar cl_bones_show_errors( "cl_bones_show_errors", "0", FCVAR_CHEAT );
...
#ifdef DEBUG
    if ( cl_bones_show_errors.GetBool() ) {
        Vector from, to;
        for ( int i = 0; i < m_pOwner->GetModelPtr()->numbones(); ++i ) {
            int const iParentIndex = m_pOwner->GetModelPtr()->boneParent( i );
            if ( iParentIndex < 0 )
                continue;

            MatrixPosition( m_pOwner->GetBone( i ), from );
            MatrixPosition( m_pOwner->GetBone( iParentIndex ), to );

            float distance = from.DistTo( to );

            if ( distance < 80.0f )
                continue;

            NDebugOverlay::Line( from, to, 255, 0, 0, true, 0.1f );
            NDebugOverlay::Text( to, UTIL_VarArgs( "%s: %i", m_pOwner->GetClassname(), i ), false, 0.1f );
        }
    }
#endif
