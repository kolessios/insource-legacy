...
void MusicImporterDialog::ImportMusic( const char* szSrcFilename, const char *szDirectory )
{
	...
	if ( g_pFullFileSystem->FileExists( fn ) )
	{
		...
		if ( FILESYSTEM_INVALID_HANDLE != hFile && FILESYSTEM_INVALID_HANDLE != hFileOut )
		{
			...
			g_SDKJukebox.AddMusicToPlaylist( szSrcFilename, hexname, id3Header.szAlbumName, id3Header.szArtistName, id3Header.szGenre );
...
void sdk_open_music_dialog_f( void )
{
	MusicImporterDialog::OpenImportDialog( null );
}

ConCommand sdk_open_music_dialog( "sdk_open_music_dialog", sdk_open_music_dialog_f, "Shows a dialog for picking custom combat music", FCVAR_DEVELOPMENTONLY );
