...
CSDKJukeboxPlaylist g_SDKJukebox;
...
static ConCommand SDK_StopJukebox( "SDK_StopJukebox", StopJukebox, "Stop the jukebox music", FCVAR_DEVELOPMENTONLY );
static ConCommand SDK_PlayRandomTrackFromJukebox( "SDK_PlayRandomTrackFromJukebox", PlayRandomTrackFromJukebox, "Play a random track from the user's jukebox", FCVAR_DEVELOPMENTONLY );
static ConVar SDK_JukeboxFadeOutTime( "SDK_JukeboxFadeOutTime", "1.0" );

void CSDKJukeboxPlaylist::AddMusicToPlaylist( const char *szFilename, const char *szHexname, const char *szAlbum, const char *szArtist, const char *szGenre )
{
...
void CSDKJukeboxPlaylist::LoadPlaylistKV()
{
...
void CSDKJukeboxPlaylist::Shutdown()
{
    StopListeningForAllEvents();
}

void CSDKJukeboxPlaylist::FireGameEvent( IGameEvent *event )
{
    if( !event )
        return;
...
void CSDKJukeboxPlaylist::PlayRandomTrack( float fadeInTime /*= 1.0f*/, const char *szDefaultTrack /*= null */ )
{
...
void CSDKJukeboxPlaylist::StopTrack( bool immediate /*= true*/, float fadeOutTime /*= 1.0f */ )
{
    if( m_pCombatMusic )
    {
        if( immediate )
...
void CSDKJukeboxPlaylist::LevelShutdownPostEntity( void )
{
    // Stop music
    StopTrack( true );
    RemoveAllMusic();
...
void CSDKJukeboxPlaylist::ExportPlayistKV( void )
{
...
void CSDKJukeboxPlaylist::SavePlaylist()
{
    Cleanup();
    ExportPlayistKV();
}

const char * CSDKJukeboxPlaylist::GetTrackName( int index )
{
...
const char * CSDKJukeboxPlaylist::GetTrackArtist( int index )
{
    if( index >= m_CombatMusicPlaylist.Count() )
        return null;
    else
...
const char * CSDKJukeboxPlaylist::GetTrackAlbum( int index )
{
    if( index >= m_CombatMusicPlaylist.Count() )
        return null;
    else
...
const char * CSDKJukeboxPlaylist::GetTrackGenre( int index )
{
    if( index >= m_CombatMusicPlaylist.Count() )
        return null;
    else
...
int CSDKJukeboxPlaylist::GetTrackCount()
{
    return m_CombatMusicPlaylist.Count();
}

void CSDKJukeboxPlaylist::RemoveMusicFromPlaylist( const char* szHexnameToRemove )
{
...
void CSDKJukeboxPlaylist::PrepareTrackKV( int index, KeyValues *pKV )
{
...
void CSDKJukeboxPlaylist::MarkTrackForDeletion( int index )
{
    if( index >= m_CombatMusicPlaylist.Count() )
        return;
    else
...
void CSDKJukeboxPlaylist::Cleanup( void )
{
...
void CSDKJukeboxPlaylist::LevelInitPostEntity( void )
{
    RemoveAllMusic();
    // Load the saved playlist
    LoadPlaylistKV();
...
void CSDKJukeboxPlaylist::RemoveAllMusic( void )
{
    StopTrack( true, 0.0f );
    m_CombatMusicPlaylist.RemoveAll();
}

bool CSDKJukeboxPlaylist::IsMusicPlaying( void )
{
