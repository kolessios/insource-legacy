...
public:
    ...
    virtual void SetID( int id ) { m_iReflectionID = id; }
...
public:
    ...
    virtual void SetID( int id ) { m_iRefractionID = id; }
	virtual bool AdjustView( float flWaterHeight );
	virtual void PushView( float waterHeight );
	virtual void PopView( );
...
void CViewRender::DrawWorldAndEntities( bool bDrawSkybox, const CViewSetup &viewIn, int nClearFlags, ViewCustomVisibility_t *pCustomVisibility )
{
	...
	{
        if ( g_ReflectiveGlassList.Count() > 0 )
        {
            // Store off view origin and angles and set the new view
            int nSaveViewID = CurrentViewID();

            FOR_EACH_VEC( g_ReflectiveGlassList, it )
            {
                cplane_t glassReflectionPlane;
                if ( IsReflectiveGlassInView( viewIn, glassReflectionPlane, g_ReflectiveGlassList[it] ) )
                {
                    SetupCurrentView( viewIn.origin, viewIn.angles, VIEW_REFLECTION );
                    ...
                    pGlassReflectionView->SetID( g_ReflectiveGlassList[it]->GetReflectionId() );
                    AddViewToScene( pGlassReflectionView );

                    SetupCurrentView( viewIn.origin, viewIn.angles, VIEW_REFRACTION );
                    ...
                    pGlassRefractionView->SetID( g_ReflectiveGlassList[it]->GetReflectionId() );
                    AddViewToScene( pGlassRefractionView );
                }
            }

            // finish off the view and restore the previous view.
            SetupCurrentView( viewIn.origin, viewIn.angles, (view_id_t)nSaveViewID );
        }
...
bool CReflectiveGlassView::AdjustView( float flWaterHeight )
{
    ...
    {
        pTexture = GetGlassReflectionTexture( m_iReflectionID );
    }
...
bool CRefractiveGlassView::AdjustView( float flWaterHeight )
{
    ...
    // Agua
    if ( m_iRefractionID == -1 )
    {
        pTexture = GetWaterRefractionTexture();
    }
    // Reflejo multiple
    else
    {
        pTexture = GetGlassRefractionTexture( m_iRefractionID );
    }
