...
void FX_Dust( const Vector &vecOrigin, const Vector &vecDirection, float flSize, float flSpeed )
{
	...
	Vector	worldLight = WorldGetLightForPoint( offset, false );
	...
	for ( int i = 0; i < numPuffs; i++ )
	{
		...
		particle.m_flDieTime  = random->RandomFloat( 0.3f, 0.6f );
        ...
        particle.m_uchStartSize = random->RandomInt( (flSize * 2.0f), (flSize * 8.0f) );
        particle.m_uchEndSize	= random->RandomInt( (flSize * 15.0f), (flSize * 35.0f) );
