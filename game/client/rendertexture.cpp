...
// Glass reflection
//=============================================================================
static CTextureReference s_pGlassReflectionTexture[MAX_REFLECTIONS];
ITexture *GetGlassReflectionTexture( int id )
{
    if ( id > MAX_REFLECTIONS )
    {
        Assert( !"ID > MAX_REFLECTIONS!" );
        return NULL;
    }

    if ( !s_pGlassReflectionTexture[id] )
    {
        s_pGlassReflectionTexture[id].Init( materials->FindTexture( UTIL_VarArgs( "_rt_GlassReflection_%i", id ), TEXTURE_GROUP_RENDER_TARGET ) );
        Assert( !IsErrorTexture( s_pGlassReflectionTexture[id] ) );
        AddReleaseFunc();
    }

    return s_pGlassReflectionTexture[id];
}

//=============================================================================
// Glass refraction
//=============================================================================
static CTextureReference s_pGlassRefractionTexture[MAX_REFLECTIONS];
ITexture *GetGlassRefractionTexture( int id )
{
    if ( id > MAX_REFLECTIONS )
    {
        Assert( !"ID > MAX_REFLECTIONS!" );
    ...
    if ( !s_pGlassRefractionTexture[id] )
    {
        s_pGlassRefractionTexture[id].Init( materials->FindTexture( UTIL_VarArgs( "_rt_GlassRefraction_%i", id ), TEXTURE_GROUP_RENDER_TARGET ) );
        Assert( !IsErrorTexture( s_pGlassRefractionTexture[id] ) );
        AddReleaseFunc();
    }

    return s_pGlassRefractionTexture[id];
}
