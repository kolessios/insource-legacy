...
void C_BaseAnimating::BuildTransformations( CStudioHdr *hdr, Vector *pos, Quaternion *q, const matrix3x4_t &cameraTransform, int boneMask, CBoneBitList &boneComputed )
{
    ...
    //Assert( g_bInThreadedBoneSetup || !ThreadInMainThread() );
...
int C_BaseAnimating::DrawModel( int flags, const RenderableInstance_t &instance )
{
	...
	if ( r_drawothermodels.GetInt() )
	{
        ...
        // Solo queremos renderizar en reflejos/sombras.
        if ( m_iVisibilityExclusivity > VISIBILITY_EXCLUSIVE_NONE )
        {
            switch ( m_iVisibilityExclusivity )
            {
                // Solo en reflejos
            case VISIBILITY_EXCLUSIVE_REFLECTION:
            {
            ...
            // Solo en sombras
            case VISIBILITY_EXCLUSIVE_SHADOWS:
            {
            ...
            // Solo en cámaras
            case VISIBILITY_EXCLUSIVE_MONITORS:
            {
            ...
            // Reflejos y sombras
            case VISIBILITY_EXCLUSIVE_REFLECTION_AND_SHADOWS:
            {
                if ( CurrentViewID() != VIEW_REFLECTION && CurrentViewID() != VIEW_SHADOW_DEPTH_TEXTURE )
                    return 0;

                break;
            }

            // En todas partes, menos el juego en si
            case VISIBILITY_EXCLUSIVE_ALL:
            {
...
void C_BaseAnimating::ProcessMuzzleFlashEvent()
{
	...
	// No hay attachment válido
	if ( iAttachment == -1 )
		return;

	Vector vecOrigin;
	GetAttachment( iAttachment, vecOrigin );

	// Make an elight
	dlight_t *el	= effects->CL_AllocDlight( index );
	el->origin		= vecOrigin;
	el->radius		= RandomFloat( 345.0f, 416.0f ); 
	el->decay		= 512.0f;
	el->die			= gpGlobals->curtime + RandomFloat( 0.03f, 0.06f );
	el->color.r		= 231;
	el->color.g		= 219;
	el->color.b		= 14;
}
...
void C_BaseAnimating::FireEvent( const Vector& origin, const QAngle& angles, int event, const char *options )
{
	...
	case AE_MUZZLEFLASH:
		{
...
#if defined ( HL2MP ) || defined ( INSOURCE_DLL ) // works for the modified CSS weapons included in the new template sdk.
			...
			{
                HACK_GETLOCALPLAYER_GUARD( "FireEvent(AE_MUZZLEFLASH)" );
	...
	{
		if ( IsPlayer() && C_BasePlayer::GetLocalPlayer() == this )
		{
			return;
		}

		DevMsg("[C_BaseAnimating::FireEvent] %s\n", options);
