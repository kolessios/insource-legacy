...
void FileWeaponInfo_t::Parse( KeyValues *pKeyValuesData, const char *szWeaponName )
{
	...
	Q_strncpy( szAnimationPrefix, pKeyValuesData->GetString( "AnimPrefix" ), MAX_WEAPON_PREFIX );

	iSlot		= pKeyValuesData->GetInt( "Bucket", 0 );
	iPosition	= pKeyValuesData->GetInt( "BucketPosition", 0 );
	...
	iMaxClip1		= pKeyValuesData->GetInt( "ClipSize", WEAPON_NOCLIP );					// Max primary clips gun can hold (assume they don't use clips by default)
	iMaxClip2		= pKeyValuesData->GetInt( "Clip2Size", WEAPON_NOCLIP );					// Max secondary clips gun can hold (assume they don't use clips by default)
	iDefaultClip1	= pKeyValuesData->GetInt( "DefaultClip", iMaxClip1 );		// amount of primary ammo placed in the primary clip when it's picked up
	iDefaultClip2	= pKeyValuesData->GetInt( "DefaultClip2", iMaxClip2 );		// amount of secondary ammo placed in the secondary clip when it's picked up
	...
	iFlags = pKeyValuesData->GetInt( "ItemFlags", ITEM_FLAG_LIMITINWORLD );
	...
	// Primary ammo used
	const char *pAmmo = pKeyValuesData->GetString( "PrimaryAmmo", "None" );
	...
	// Secondary ammo used
	pAmmo = pKeyValuesData->GetString( "SecondaryAmmo", "None" );
	if ( strcmp("None", pAmmo) == 0)
