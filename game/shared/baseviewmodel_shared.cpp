...
bool CBaseViewModel::IsHands()
{
    return (GetViewModel() != NULL && IsEffectActive( EF_BONEMERGE ));
}
...
// Input  : nIndex - 
//-----------------------------------------------------------------------------
void CBaseViewModel::SetOwnerViewModel( CBaseViewModel * pViewmodel )
{
    m_hOwnerViewModel = pViewmodel;

    SetOwnerEntity( pViewmodel );
...
void CBaseViewModel::SendViewModelMatchingSequenceLayer( int sequence )
{
	// @TODO: Client-side
#ifndef CLIENT_DLL
	AddGestureSequence( sequence );
...
BEGIN_NETWORK_TABLE_NOBASE(CBaseViewModel, DT_BaseViewModel)
    ...
    SendPropEHandle( SENDINFO( m_hOwnerViewModel ) ),
    ...
    RecvPropEHandle( RECVINFO( m_hOwnerViewModel ) ),
