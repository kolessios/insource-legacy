...
#ifdef GAME_DLL // don't reset on the client until a ragdoll has been generated
	ResetGestureSlots();
#endif
}
	...
	case PLAYERANIMEVENT_JUMP:
		{
			...
			RestartGesture( GESTURE_SLOT_JUMP, ACT_MP_JUMP );
...
void CMultiPlayerAnimState::RunGestureSlotAnimEventsToCompletion( GestureSlot_t *pGesture )
{
	...
	{
		mstudioevent_t* pevent = (mstudioevent_t*)seqdesc.pEvent( 0 );
		...
		for (int i = 0; i < (int)seqdesc.numevents; i++)
		{
			...
			else if ( pevent[i].szeventindex < 5000 ) //Adrian - Support the old event system
				continue;

			if ( pevent[i].cycle > pGesture->m_pAnimLayer->GetPrevCycle() &&
				pevent[i].cycle <= pGesture->m_pAnimLayer->GetCycle() )
			{
				pPlayer->FireEvent( pPlayer->GetAbsOrigin(), pPlayer->GetAbsAngles(), pevent[ i ].szeventindex, pevent[ i ].pszOptions() );
			}
		}
	}
}
...
bool CMultiPlayerAnimState::HandleMoving( Activity &idealActivity )
{
	...
	if ( vel.LengthSqr() > MOVING_MINIMUM_SPEED )
	{
		// Always assume a run.
...
void CMultiPlayerAnimState::ComputePoseParam_MoveYaw( CStudioHdr *pStudioHdr )
{
...
#ifdef CLIENT_DLL
	vel = GetBasePlayer()->GetLocalVelocity();
	QAngle velAng;
	VectorAngles( vel, velAng );
	flYaw = AngleNormalize( -(GetBasePlayer()->GetRenderAngles().y - velAng.y) );
	engine->Con_NPrintf( 10, "%f - %f", flYaw, vel.Length() );
#endif

	// Get the current speed the character is running.
	bool bIsMoving = vel.LengthSqr() > 1.0f;
	 //CalcMovementPlaybackRate( &bIsMoving );
	...
	if ( bIsMoving )
	{
		...
		float flPlaybackRate = vel.Length() / 200.0f;
	...
	vecCurrentMoveYaw.x = vel.Length() / 200.0f;
	engine->Con_NPrintf( 12, "%f - %f - %f", vecCurrentMoveYaw.x, vecCurrentMoveYaw.y, GetBasePlayer()->GetCycle() );
#endif
	...
	//GetBasePlayer()->SetPlaybackRate( 1 );
...
#ifdef CLIENT_DLL
	//angles = GetBasePlayer()->GetRenderAngles();
#endif
