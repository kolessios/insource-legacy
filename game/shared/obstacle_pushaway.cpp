...
//-----------------------------------------------------------------------------------------------------
ConVar sv_pushaway_force( "sv_pushaway_force", "2500", FCVAR_REPLICATED, "How hard physics objects are pushed away from the players on the server." );
...
void PerformObstaclePushaway( CBaseCombatCharacter *pPushingEntity )
{
	...
	for ( int i=0; i < nEnts; i++ )
	{
        if ( props[i] == pPushingEntity )
            continue;

        if ( !TheGameRules->CanPushEntity( pPushingEntity, props[i] ) )
            continue;
		...
		{		
            ...
            if ( props[i]->IsPlayer() )
            {
                ...
                props[i]->ApplyAbsVelocityImpulse( vPushAway * flForce );
            }
            else
            {
